      ...
      <CardContent>
        {loading ? (
          <div className="flex items-center gap-2">
            <Loader2 className="h-5 w-5 animate-spin text-hotpink-500" />
            <span className="text-muted-foreground">Loading...</span>
          </div>
        ) : (
          <>
            <div className="text-2xl font-bold bg-gradient-to-r from-hotpink-600 to-purple-600 bg-clip-text text-transparent">
              {value}
            </div>
            {change !== undefined && (
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-hotpink-500 to-purple-500 bg-clip-text text-transparent">
            Dashboard
          </h1>
      ...
      {error && (
        <div className="bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800 rounded-lg p-4 text-rose-600 dark:text-rose-400">
          {error}
        </div>
      )}
          ...
          <CardContent>
            {loading ? (
              <div className="flex items-center justify-center py-8">
                <Loader2 className="h-6 w-6 animate-spin text-hotpink-500" />
              </div>
            ) : agents.length === 0 ? (
              ...
              <div className="space-y-4">
                {agents.map((agent) => (
                  <div
                    key={agent.userId}
                    className="flex items-center gap-4 p-2 rounded-xl hover:bg-hotpink-50 dark:hover:bg-purple-900/30 transition-all duration-200"
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {agent.messagesOut} messages sent · {agent.conversationsClosed} closed
                      </p>
                    </div>
                  </div>
                ))}
                  ...
                  return (
                    <div key={channel.channelId} className="space-y-2">
                      ...
                      <div className="flex text-xs text-muted-foreground gap-4">
                        <span>In: {channel.messagesIn}</span>
                        <span>Out: {channel.messagesOut}</span>
